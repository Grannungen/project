    ...
    var syncObject = $firebaseObject(ref);
   	...
   	syncObject.$bindTo($rootScope, "syncObject").then(function () {
		for (var i = 0; i < firebaseArray.length; i++) {
  			...
  			var day = meetingAgendaModel.addDay(firebaseArray[i]);
			if (firebaseArray[i].activities != undefined){
				for (var j=0; j < firebaseArray[i].activities.length;j++){
					day._addActivity(new meetingAgendaModel.Activity(firebaseArray[i].activities[j]));
				}
			}
		}
